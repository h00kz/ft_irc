...
int main(int argc, char *argv[])
{
    ...
    while (true)
    {
        ...
        if (strstr(msg, "PRIVMSG") != NULL)
        {
            ...
            message = req.substr(pos + 1, strlen(msg));
            ...
            if (menuDisplayed && strstr(message.c_str(), "!menu") != NULL && !gameRunning)
            {
				formatMsg = "PRIVMSG " + sender + " :Menu:\r\n";
                send(clientSd, formatMsg.c_str(), formatMsg.length(), 0);
				formatMsg = "PRIVMSG " + sender + " :1. Jouer au jeu du plus ou moins\r\n";
                send(clientSd, formatMsg.c_str(), formatMsg.length(), 0);
				formatMsg = "PRIVMSG " + sender + " :2. Quitter\r\n";
                send(clientSd, formatMsg.c_str(), formatMsg.length(), 0);
				formatMsg = "PRIVMSG " + sender + " :Entrez le numéro de l'option que vous souhaitez choisir.\r\n";
                send(clientSd, formatMsg.c_str(), formatMsg.length(), 0);
            }
            else if (menuDisplayed && strstr(message.c_str(), "1") != NULL && !gameRunning)
            {
                gameRunning = true;
            ...
            else if (menuDisplayed && strstr(message.c_str(), "2") != NULL && !gameRunning)
            {
