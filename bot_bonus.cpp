	...
	while (true)
	{
		...
		if (strstr(msg, "PRIVMSG") != NULL)
		{
			...
			if (gameRunningPM && !menuDisplayed)
			{
				...
				try {
					if (message.at(0) == '\r' || message.at(0) == '0')
					{
						formatMsg = "PRIVMSG "+ sender + " :Entrez un nombre entre 1 et 100\r\n";
						send(clientSd, formatMsg.c_str(), formatMsg.length(), 0);
						continue;
					}
			...
			else if (strstr(message.c_str(), "!menu") != NULL && !gameRunningPM && !gameRunningPFC)
			{
				menuDisplayed = true;
